...
export const JobInputSchema = z.discriminatedUnion("jobType", [
  z.object({
    jobType: z.literal("broken_link_check"),
    input: z.object({ urls: z.array(z.string().url()) }),
  }),
  z.object({
    jobType: z.literal("email_validation"),
    input: z.object({ emails: z.array(z.string().email()) }),
  }),
  z.object({
    jobType: z.literal("webpage_to_markdown"),
    input: z.object({ url: z.string().url() }),
  }),
  z.object({
    jobType: z.literal("domain_availability"),
    input: z.object({ domains: z.array(z.string().min(1)) }),
  }),
  z.object({
    jobType: z.literal("keyword_extraction"),
    input: z.object({ text: z.string().min(1) }),
  }),
  z.object({
    jobType: z.literal("language_detection"),
    input: z.object({ texts: z.array(z.string().min(1)) }),
  }),
  z.object({
    jobType: z.literal("readability_score"),
    input: z.object({ text: z.string().min(1) }),
  }),
  z.object({
    jobType: z.literal("duplicate_detection"),
    input: z.object({ strings: z.array(z.string().min(1)) }),
  }),
]);
